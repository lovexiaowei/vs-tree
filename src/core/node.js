...
import { insterAfter, onDragEnterGap, parseTemplate, removeClass } from './utils.ts'
...
export default class Node {
  ...
  createNode () {
    ...
    dom.addEventListener('click', (e) => {
      ...
      if (this.store.multiDrag && (shiftKey || ctrlKey)) {
        ...
        if (shiftKey) {
          this.store.selectManager.shiftAdd(this, this.store.selectedCurrent)
        }
      } else {
  ...
  createDraggable (dom) {
    ...
    dom.addEventListener('dragover', (e) => {
      ...
      const halfH = (this.store.itemHeight || 26) / 2
      const gap = e.clientY - this.dom.getBoundingClientRect().top - halfH > 0 ? 1 : -1
      // console.log(gap);
      ...
      if (gap === 1) {
        ...
        this.dom.classList.add('vs-drag-over-node-bottom')
        return
      }
      if (!this.isLeaf) {
        this.dom.classList.add('vs-drag-enter')
      }
    })
