  ...
  _createClass(Node, [{
    ...
    value: function insertChild(child, index) {
      ...
      if (typeof index === 'undefined' || index < 0) {
        this.childNodes.push(child); // this.data.children&&this.data.children.push(child.data);
      } else {
        this.childNodes.splice(index, 0, child); // this.data.children&&this.data.children.splice(index,0,child.data);
      }
    ...
    value: function createDragable(dom) {
      ...
      dom.addEventListener('dragenter', function (e) {
        e.stopPropagation();
        e.preventDefault();
        _this8.store.dropNode && removeClass(_this8.store.dropNode);

        if (_this8.store.dragNode.dom === _this8.dom) {
          //如果放到自己上面，那么不做处理
          _this8.store.dropNode = null;
          return;
        }
      ...
      function removeClass(dom) {
        if (!dom) return;
        dom.classList.remove('vs-drag-over-node'); // dom.classList.remove('vs-drag-enter')
      ...
      dom.addEventListener('drop', function (e) {
        // debugger;
        e.stopPropagation();
        if (!_this8.store.dropNode) return;

        if (_this8.store.canDrop && _this8.store.canDrop(_this8.store.dragNode, _this8) === false) {
          removeClass(_this8.store.dropNode);
          return false;
        } // this.store.onDrop(e, this, this.store.dropPostion)


        _this8.store.onDrop(e, _this8.store.dragNode, _this8); // console.log(this.store.dragNode,this);
        ...
        if (_this8.store.dropable) {
          ...
          if (dragNode && _this8.parent) {
            ...
            // dragNode.childNodes
            var son = [];
            dig(dragNode.childNodes); // dragNode.childNodes.map(v=>{
            //     if (v.data) son.push(v.data.id);
            // });
            // debugger;
            // console.log(son,this.data.id);

            if (son.indexOf(_this8.data.id) > -1) {
              return;
            }
            ...
            if (_this8.isLeaf === false) {
              _this8.unshift(data);
            } else if ([1, -1].indexOf(_this8.store.dropPostion) > -1) {
